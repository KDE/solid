...
{
    // supported since UPower 1.90.5 (2024-08-26)
...
{
    // Supported by a upower > 1.90.9:
    // https://gitlab.freedesktop.org/upower/upower/-/merge_requests/276#note_3034202
    const unsigned int startThresholdSupported = 0x1;
    QVariant enabled = m_device.data()->prop(QStringLiteral("ChargeThresholdSettingsSupported"));
    return enabled.isValid() ? (enabled.toUInt() & startThresholdSupported) : chargeLimitSupported();
}

int Battery::chargeStartThreshold() const
{
    bool ok = false; // supported since UPower 1.90.5 (2024-08-26)
...
bool Battery::chargeEndThresholdSupported() const
{
    // Supported by a upower > 1.90.9:
    // https://gitlab.freedesktop.org/upower/upower/-/merge_requests/276#note_3034202
    const unsigned int endThresholdSupported = 0x2;
    QVariant enabled = m_device.data()->prop(QStringLiteral("ChargeThresholdSettingsSupported"));
    return enabled.isValid() ? (enabled.toUInt() & endThresholdSupported) : chargeLimitSupported();
}
